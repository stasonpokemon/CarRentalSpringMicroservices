...
public interface OrderController {
    ...
    @Operation(
            summary = "Find orders by user's id",
            description = "This endpoint allows you to get order's by user's id from database")
    @ApiResponses({
            @ApiResponse(responseCode = "200",
                    description = "Found the following orders",
                    content = {@Content(mediaType = "application/json",
    ...
    @GetMapping("/user/{id}")
    ResponseEntity<?> findAllByUserId(@PathVariable("id") UUID userId,
                                      @ParameterObject @PageableDefault Pageable pageable);
